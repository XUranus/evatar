...
DEFAULT_SKILLS = [
    {
        "id": "summarize",
        "name": "总结截图",
        "description": "分析最近的截图，生成摘要笔记",
        "system_prompt": "请帮我整理最近截图的内容，按主题分类，生成一份简洁的笔记摘要。",
        "icon": "📝",
    },
    {
        "id": "reminders",
        "name": "提取提醒",
        "description": "从截图中提取所有需要提醒的事项",
        "system_prompt": "请搜索我的截图知识库，找出所有包含时间、日期、截止时间的内容，整理成一个提醒事项列表。",
        "icon": "⏰",
    },
    {
        "id": "research",
        "name": "深度研究",
        "description": "对截图中的主题进行深入研究",
        "system_prompt": "请搜索我最近截图中关注的主题，然后用互联网搜索补充最新信息，给我一份完整的研究报告。",
        "icon": "🔬",
    },
    {
        "id": "stock",
        "name": "股票分析",
        "description": "整理截图中的股票和金融信息",
        "system_prompt": "请搜索我截图中的股票、基金、金融相关内容，结合互联网搜索最新行情，整理成投资笔记。",
        "icon": "📈",
    },
]
...
def list_skills(db: Session = Depends(get_db)):
    skills = db.query(Skill).filter(Skill.enabled == 1).all()
    if not skills:
        # Seed default skills
...
def get_skill(skill_id: str, db: Session = Depends(get_db)):
    ...
    return {"id": skill.id, "name": skill.name, "description": skill.description, "system_prompt": skill.system_prompt, "icon": skill.icon}
...
def list_mcp_servers(db: Session = Depends(get_db)):
    ...
    return {"servers": [{"id": s.id, "name": s.name, "url": s.url, "description": s.description, "enabled": s.enabled} for s in servers]}
